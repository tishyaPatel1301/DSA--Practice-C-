# DSA Practice Collection — C++

Beginner DSA programs in C++ covering Arrays, Sorting, Searching, Patterns, Strings, and Math.
Built during B.E. CSE Semester 2

## Topics Covered
- Arrays (reverse, max/min, duplicates, second largest)
- Sorting (bubble, selection, insertion)
- Searching (linear, binary)
- Patterns (pyramid, diamond, number triangle)
- Strings (reverse, palindrome, vowel count, frequency)
- Math (factorial, fibonacci, prime, armstrong)

## How to Run
g++ filename.cpp -o output
./output

## Author
Tishya Patel | B.E. CSE | NLJIET, Ahmedabad
